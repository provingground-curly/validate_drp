...
def loadOneCatalog(dataId, butler, ccdKeyName, schema, mapper,
    ...
    if useJointCal:
        try:
            photoCalib = butler.get("jointcal_photoCalib", dataId)
        except (FitsError, dafPersist.NoResults) as e:
            print(e)
            print("Could not open photometric calibration for ", dataId)
            print("Skipping this dataId.")
            return None
        try:
            wcs = butler.get("jointcal_wcs", dataId)
        except (FitsError, dafPersist.NoResults) as e:
            print(e)
            print("Could not open updated WCS for ", dataId)
            print("Skipping this dataId.")
            return None
    else:
        wcs = None
